        ...
        {publishedEvents.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {publishedEvents.map(event => (
              <div key={event.id} className="bg-white dark:bg-slate-900 border rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-shadow group">
                <div className="p-6 flex flex-col h-full">
                  <div className="text-xs font-bold text-green-600 uppercase tracking-wider mb-2">
                    {event.division?.name || 'Umum'}
                  </div>
                  <h3 className="text-xl font-bold text-gray-900 dark:text-gray-100 mb-3 group-hover:text-green-700 dark:text-green-400 transition-colors">{event.name}</h3>
                  <p className="text-sm text-gray-600 dark:text-gray-400 mb-4 line-clamp-3 flex-grow">{event.description}</p>
          ...
          <div className="text-center p-12 bg-white dark:bg-slate-900 border border-dashed rounded-xl shadow-sm">
            ...
            <h3 className="text-lg font-medium text-gray-900 dark:text-gray-100">Belum ada kegiatan</h3>
            <p className="mt-1 text-gray-500 dark:text-gray-400">Tidak ada agenda kegiatan yang dipublikasikan saat ini.</p>
          </div>
        )}
      </div>
    </div>
