  ...
  return (
    <>
      <div className="relative max-w-4xl mx-auto py-10">
        {/* Vertical line */}
        <div className="absolute left-6 md:left-1/2 top-0 bottom-0 w-1 bg-green-200 dark:bg-green-900/50 -translate-x-1/2 rounded-full pointer-events-none"></div>
          ...
          {programs.map((program, index) => {
            ...
            return (
              <div key={program.id} className="relative flex items-center md:justify-between flex-col md:flex-row">
                {/* Circle Marker */}
                <button
                  className="absolute left-6 md:left-1/2 w-8 h-8 rounded-full bg-green-500 border-4 border-gray-50 dark:border-slate-950 shadow-md transform -translate-x-1/2 flex items-center justify-center z-20 hover:scale-125 transition-transform focus:outline-none focus-visible:ring-2 focus-visible:ring-green-400"
                  ...
                  >
                    ...
                    {program.customStatus && (
                      <div className={`flex items-center gap-2 mb-3 ${isLeft ? "md:justify-end" : "md:justify-start"}`}>
                        ...
                        <span className="text-sm font-medium text-gray-700 dark:text-gray-300">{program.customStatus}</span>
                      </div>
                    )}
      ...
      {/* Portal Modal – always at document.body, always centered in viewport */}
