        ...
        {publishedPrograms.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {publishedPrograms.map(program => (
              <div key={program.id} className="bg-white dark:bg-slate-900 rounded-xl shadow-sm hover:shadow-md transition-shadow border p-6 flex flex-col h-full border-t-4 border-t-green-600">
                <div className="text-xs font-bold text-green-600 uppercase tracking-wider mb-2">
                  {program.division?.name || 'Umum'}
                </div>
                <h3 className="text-xl font-bold text-gray-900 dark:text-gray-100 mb-3">{program.name}</h3>
                <p className="text-gray-600 dark:text-gray-400 mb-4 flex-grow text-sm leading-relaxed whitespace-pre-wrap">
                  {program.description}
                </p>
                {program.objective && (
                  <div className="mt-4 pt-4 border-t border-gray-100 dark:border-slate-800">
                    <h4 className="text-sm font-semibold text-gray-900 dark:text-gray-100 mb-2">Tujuan:</h4>
                    <p className="text-sm text-gray-600 dark:text-gray-400 whitespace-pre-wrap">{program.objective}</p>
                  </div>
                )}
              </div>
            ))}
