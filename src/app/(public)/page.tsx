...
export default async function HomePage() {
  ...
  return (
    <div className="bg-gray-50 dark:bg-slate-950">
      ...
      {/* Hero Section */}
      <section className="relative min-h-[85vh] lg:min-h-screen flex flex-col justify-end pt-32 pb-24 lg:pb-32 overflow-hidden bg-slate-950 text-white">
        ...
        <div className="container mx-auto px-4 lg:px-8 relative z-10">
          <div className="max-w-4xl mx-auto text-center space-y-6 lg:space-y-8 mt-auto">
            <div className="inline-block px-4 py-1.5 rounded-full bg-black/30 border border-white/20 backdrop-blur-md text-green-50 font-semibold text-sm mb-2 lg:mb-4 animate-fade-in-up shadow-lg">
              <span className="w-2 h-2 rounded-full bg-green-400 animate-pulse inline-block mr-2"></span>
              UKM Kerohanian Islam STMIK IKMI
            </div>
            <h1 className="text-4xl md:text-5xl lg:text-7xl font-extrabold tracking-tight leading-[1.1] animate-fade-in-up drop-shadow-2xl" style={{ animationDelay: '0.1s' }}>
            ...
            <p className="text-base md:text-lg lg:text-xl text-gray-100 max-w-2xl mx-auto leading-relaxed animate-fade-in-up drop-shadow-lg font-medium" style={{ animationDelay: '0.2s' }}>
              Wadah pembinaan mahasiswa muslim STMIK IKMI Cirebon untuk mengembangkan potensi spiritual, intelektual, dan sosial.
            </p>
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4 pt-4 lg:pt-6 animate-fade-in-up" style={{ animationDelay: '0.3s' }}>
              <Link href="/rekrutmen" className="w-full sm:w-auto px-8 py-3.5 rounded-full bg-green-600 text-white font-bold hover:bg-green-500 hover:scale-105 hover:shadow-xl hover:shadow-green-500/30 transition-all duration-300 active:scale-95 shadow-lg">
                Bergabung Sekarang
              </Link>
              <Link href="/profil" className="w-full sm:w-auto px-8 py-3.5 rounded-full bg-black/40 border border-white/30 backdrop-blur-md text-white font-semibold hover:bg-black/60 transition-all duration-300 active:scale-95">
                Kenali Kami
              </Link>
            </div>
          </div>
      ...
      {/* Ticker / Running Text */}
      <section className="pt-12 pb-6 bg-gray-50 dark:bg-slate-950 dark:bg-slate-900 border-b border-gray-200 dark:border-slate-700/60 dark:border-slate-800 overflow-hidden transition-colors duration-300">
        ...
        <div className="container mx-auto px-4 lg:px-8">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end mb-10 gap-4">
            <div>
              ...
              <h2 className="text-3xl md:text-4xl font-extrabold text-gray-900 dark:text-gray-100 dark:text-white mb-2 tracking-tight">Kegiatan Terdekat</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {latestEvents.length > 0 ? latestEvents.map(event => (
              <div key={event.id} className="bg-white dark:bg-slate-900 dark:bg-slate-800 border border-gray-100 dark:border-slate-800 dark:border-slate-700 rounded-2xl overflow-hidden shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group">
                <div className="p-6 md:p-8">
                  ...
                  <h3 className="text-xl font-bold text-gray-900 dark:text-gray-100 dark:text-white mb-3 group-hover:text-green-600 dark:group-hover:text-green-400 transition-colors line-clamp-2">{event.name}</h3>
                  <p className="text-sm text-gray-600 dark:text-gray-400 dark:text-gray-300 mb-5 line-clamp-2 leading-relaxed">{event.description}</p>
                  
                  <div className="space-y-2 mt-auto">
                    <div className="flex items-center text-sm text-gray-600 dark:text-gray-400 bg-gray-50 dark:bg-slate-950 dark:bg-slate-900/50 p-2 rounded-lg">
              ...
              <div className="col-span-full flex flex-col items-center justify-center text-center py-16 bg-white dark:bg-slate-900/60 dark:bg-slate-800/40 backdrop-blur-sm rounded-2xl border border-dashed border-gray-200 dark:border-slate-700 shadow-sm">
                <div className="w-16 h-16 bg-gray-100 dark:bg-slate-800 dark:bg-slate-700 rounded-full flex items-center justify-center mb-4">
                  <svg className="w-8 h-8 text-gray-400 dark:text-gray-500 dark:text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
                </div>
                <h3 className="text-lg font-bold text-gray-900 dark:text-gray-100 dark:text-white mb-1">Belum Ada Kegiatan</h3>
        ...
        <div className="container mx-auto px-4 lg:px-8">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end mb-10 gap-4">
            <div>
              ...
              <h2 className="text-3xl md:text-4xl font-extrabold text-gray-900 dark:text-gray-100 dark:text-white mb-2 tracking-tight">Berita & Artikel</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {latestArticles.length > 0 ? latestArticles.map(article => (
              <Link href={`/berita/${article.slug}`} key={article.id} className="bg-white dark:bg-slate-900 dark:bg-slate-800 border border-gray-100 dark:border-slate-800 dark:border-slate-700 rounded-2xl overflow-hidden shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group flex flex-col">
                <div className="relative h-56 bg-gray-200 dark:bg-slate-700 w-full overflow-hidden">
                  {article.coverImage ? (
                    ...
                    <div className="w-full h-full flex items-center justify-center text-gray-400 dark:text-gray-500 dark:text-gray-400 bg-gray-100 dark:bg-slate-800/80">
                ...
                <div className="p-6 md:p-8 flex-grow flex flex-col">
                  ...
                  <h3 className="text-xl font-bold text-gray-900 dark:text-gray-100 dark:text-white mb-3 group-hover:text-blue-600 dark:text-blue-400 dark:group-hover:text-blue-400 transition-colors line-clamp-2 leading-tight">{article.title}</h3>
                  ...
                  <div className="mt-auto pt-4 border-t border-gray-100 dark:border-slate-800 dark:border-slate-700/50 flex items-center justify-between">
              ...
              <div className="col-span-full flex flex-col items-center justify-center text-center py-16 bg-gray-50 dark:bg-slate-950/50 dark:bg-slate-800/30 rounded-2xl border border-dashed border-gray-200 dark:border-slate-700 shadow-sm">
                <div className="w-16 h-16 bg-white dark:bg-slate-900 dark:bg-slate-700 rounded-full flex items-center justify-center mb-4 shadow-sm">
                ...
                <h3 className="text-lg font-bold text-gray-900 dark:text-gray-100 dark:text-white mb-1">Belum Ada Berita</h3>
