...
export async function GET(req: Request) {
  try {
    ...
    const data = await db.select().from(pensosKajianKelas);
...
export async function POST(req: Request) {
  try {
    ...
    const [newData] = await db.insert(pensosKajianKelas).values(body).returning();
...
export async function DELETE(req: Request) {
  try {
    ...
    await db.delete(pensosKajianKelas);
