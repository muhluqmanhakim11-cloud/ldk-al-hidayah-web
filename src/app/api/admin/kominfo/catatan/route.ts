...
export async function GET(req: Request) {
  try {
    ...
    if (user.realRole !== 'super_admin' && user.realRole !== 'admin_kominfo') {
       return NextResponse.json({ error: "Forbidden" }, { status: 403 });
    }

    // Get division ID for kominfo
    const divs = await db.select().from(divisions).where(eq(divisions.slug, 'kominfo'));
