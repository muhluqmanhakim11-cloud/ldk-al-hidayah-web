...
export async function GET(req: Request) {
  try {
    ...
    const data = await db.select().from(kaderMentoringAbsensi);
...
export async function POST(req: Request) {
  try {
    ...
    const [newData] = await db.insert(kaderMentoringAbsensi).values(body).returning();
...
export async function DELETE(req: Request) {
  try {
    ...
    await db.delete(kaderMentoringAbsensi);
