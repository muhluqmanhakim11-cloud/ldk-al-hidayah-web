...
export async function GET(req: Request) {
  try {
    ...
    const data = await db.select().from(kaderDatabase);
...
export async function POST(req: Request) {
  try {
    ...
    const [newData] = await db.insert(kaderDatabase).values(body).returning();
    
    
    try {
      await logActivity({
        action: "CREATE",
        entityType: "KADERISASI_DATABASE",
        entityName: "Data",
