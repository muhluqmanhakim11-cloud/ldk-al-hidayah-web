...
export default async function RecruitmentDetailPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    return (
      <div className="p-6">
        <div className="bg-red-50 text-red-600 p-6 rounded-xl border border-red-200">
          ...
          <Link href="/admin/recruitment" className="inline-block mt-4 text-red-800 underline">Kembali ke Daftar</Link>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="p-6">
      <div className="mb-6">
        <Link href="/admin/recruitment" className="text-sm text-gray-500 hover:text-green-600 mb-2 inline-flex items-center">
          <svg className="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 19l-7-7m0 0l7-7m-7 7h18" /></svg>
          Kembali
        </Link>
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold text-gray-900">Detail Pendaftar: {data.name}</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white p-6 rounded-xl shadow-sm border">
            <h3 className="font-bold text-gray-900 mb-4 pb-2 border-b">Biodata Lengkap</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                <p className="text-xs text-gray-500 font-semibold uppercase">NIM</p>
                <p className="text-gray-900 font-medium">{data.nim}</p>
              </div>
              <div>
                <p className="text-xs text-gray-500 font-semibold uppercase">Program Studi</p>
                <p className="text-gray-900 font-medium">{data.studyProgram}</p>
              </div>
              <div>
              ...
              <div>
                ...
                <p className="text-gray-900 font-medium"><a href={`https://wa.me/${data.whatsapp?.replace(/^0/, '62')}`} target="_blank" rel="noreferrer" className="text-green-600 hover:underline">{data.whatsapp}</a></p>
              </div>
              <div className="md:col-span-2">
                <p className="text-xs text-gray-500 font-semibold uppercase">Divisi Diminati</p>
                <p className="text-gray-900 font-bold">{data.interestedDivision?.name || '-'}</p>
              </div>
              <div className="md:col-span-2">
                <p className="text-xs text-gray-500 font-semibold uppercase">Alasan Bergabung</p>
                <p className="text-gray-900 mt-1 whitespace-pre-wrap p-3 bg-gray-50 rounded border">{data.reason}</p>
              </div>
            </div>
          </div>

          <div className="bg-white p-6 rounded-xl shadow-sm border">
            <h3 className="font-bold text-gray-900 mb-4 pb-2 border-b">Riwayat Perubahan Status</h3>
        ...
        <div className="space-y-6">
          <div className="bg-white p-6 rounded-xl shadow-sm border text-center">
            <h3 className="font-bold text-gray-900 mb-4 pb-2 border-b text-left">Pas Foto</h3>
            {data.photoUrl ? (
              ...
              <div className="w-48 h-64 mx-auto bg-gray-100 rounded-lg flex items-center justify-center text-gray-400 border border-dashed">
                Tidak ada foto
              </div>
            )}
          </div>
