...
export default function ClientPage() {
  ...
  const columns = [
    ...
    { header: "Role Akses", accessor: (row: any) => (
      <span className={`px-2 py-1 rounded text-xs font-medium ${row.role === 'super_admin' ? 'bg-purple-100 text-purple-700' : 'bg-blue-100 text-blue-700 dark:text-blue-400'}`}>
    ...
    { header: "Terdaftar", accessor: (row: any) => new Date(row.createdAt as string).toLocaleDateString("id-ID") },
    {
      ...
      accessor: (row: any) => (
        <div className="flex gap-2">
          <button onClick={() => handleDelete(row.id)} className="text-red-500 hover:text-red-700 dark:text-red-400 p-1 bg-red-50 dark:bg-red-900/30 rounded" title="Hapus User">
            <Trash2 size={16} />
          </button>
        </div>
      )
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-4 bg-white dark:bg-slate-900 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-800">
        <div>
          <h1 className="text-2xl font-bold text-gray-800 dark:text-gray-200">Manajemen Pengguna</h1>
          <p className="text-gray-500 dark:text-gray-400 mt-1">Kelola akun admin dan hak akses divisi</p>
        </div>
