...
export default function ClientPage() {
  ...
  const fetchData = async () => {
    setIsLoading(true);
    try {
      const res = await fetch("/api/admin/seni-olahraga/catatan");
  ...
  const handleDeleteAll = async () => {
    ...
    try {
      const res = await fetch("/api/admin/seni-olahraga/catatan", { method: "DELETE" });
      if (res.ok) {
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const res = await fetch(`/api/admin/seni-olahraga/catatan/${id}`, { method: "DELETE" });
      if (res.ok) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-4 bg-white dark:bg-slate-900 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-800">
        <div>
          <h1 className="text-2xl font-bold text-gray-800 dark:text-gray-200">Catatan Seni & Olahraga</h1>
