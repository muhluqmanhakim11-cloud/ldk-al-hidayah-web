...
export default function ClientPage() {
  ...
  const columns = [
    ...
    { header: "Status", accessor: (row: any) => (
      <span className={`px-2 py-1 rounded text-xs font-medium ${row.status === 'Selesai' ? 'bg-green-100 text-green-700 dark:text-green-400' : 'bg-yellow-100 text-yellow-700'}`}>
        {row.status}
      </span>
    ) },
    {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-4 bg-white dark:bg-slate-900 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-800">
        <div>
          <h1 className="text-2xl font-bold text-gray-800 dark:text-gray-200">Kunjungan Tokoh & Ulama</h1>
          <p className="text-gray-500 dark:text-gray-400 mt-1">Penjadwalan & rekaman kunjungan silaturahmi</p>
        </div>
