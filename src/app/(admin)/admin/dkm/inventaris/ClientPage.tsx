  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center bg-white dark:bg-slate-900 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-800 print:hidden">
        <div>
          <h1 className="text-2xl font-bold text-gray-800 dark:text-gray-200">Inventaris Masjid</h1>
          <p className="text-gray-500 dark:text-gray-400 mt-1">Kelola data inventaris masjid</p>
        </div>
        <div className="flex flex-wrap gap-3 w-full md:w-auto">
          <button onClick={() => window.print()} className="bg-gray-100 dark:bg-slate-800 hover:bg-gray-200 dark:bg-slate-700 text-gray-700 dark:text-gray-300 px-5 py-2.5 rounded-xl flex items-center gap-2 transition-all shadow-sm"><Printer size={20} /> Cetak PDF</button>
...
<button
      ...
      <Modal isOpen={isFormOpen} onClose={() => setIsFormOpen(false)} title="Tambah Data Baru">
        <form className="space-y-4 mt-4" onSubmit={handleSubmit}>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Tanggal Audit</label>
