    ...
    { 
      ...
      accessor: (row: any) => (
        <span className={`px-2 py-1 text-xs font-semibold rounded-md 
          ${row.action === 'CREATE' ? 'bg-green-100 text-green-700 dark:text-green-400' : 
            row.action === 'UPDATE' ? 'bg-blue-100 text-blue-700 dark:text-blue-400' : 
            row.action === 'DELETE' ? 'bg-red-100 text-red-700 dark:text-red-400' : 'bg-gray-100 dark:bg-slate-800 text-gray-700 dark:text-gray-300'}`}>
          {row.action}
        </span>
      ) 
    },
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white dark:bg-slate-900 p-4 rounded-xl shadow-sm border border-gray-100 dark:border-slate-800 flex flex-col md:flex-row gap-4 items-center justify-between">
        <div className="flex flex-col sm:flex-row gap-4 w-full md:w-auto">
          <select 
            className="border-gray-300 dark:border-slate-600 dark:border-slate-700 rounded-lg px-4 py-2 text-sm w-full md:w-64 bg-gray-50 dark:bg-slate-950 dark:bg-slate-800 dark:text-gray-100 focus:ring-blue-500 focus:border-blue-500" 
