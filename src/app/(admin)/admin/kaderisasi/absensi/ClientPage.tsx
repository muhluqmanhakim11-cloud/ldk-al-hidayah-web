...
export default function ClientPage() {
  ...
  const [formData, setFormData] = useState({ namaHalaqah: '', mentor: '', tanggal: new Date().toISOString().slice(0,10), materi: '', daftarHadir: '', evaluasi: '' });

  const fetchData = async () => {
    setIsLoading(true);
    try {
      const res = await fetch("/api/admin/kaderisasi/absensi");
  ...
  const handleDeleteAll = async () => {
    ...
    try {
      const res = await fetch("/api/admin/kaderisasi/absensi", { method: "DELETE" });
      if (res.ok) {
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const res = await fetch(`/api/admin/kaderisasi/absensi/${id}`, { method: "DELETE" });
      if (res.ok) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setIsFormOpen(false);
        fetchData();
        setFormData({ namaHalaqah: '', mentor: '', tanggal: new Date().toISOString().slice(0,10), materi: '', daftarHadir: '', evaluasi: '' });
      } else {
  ...
  const columns = [
    ...
    { header: "Nama Halaqah", accessor: "namaHalaqah" },
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-4 bg-white dark:bg-slate-900 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-800">
        <div>
          <h1 className="text-2xl font-bold text-gray-800 dark:text-gray-200">Absensi Mentoring</h1>
          <p className="text-gray-500 dark:text-gray-400 mt-1">Kelola data absensi mentoring</p>
        </div>
...
<button
      ...
      <Modal isOpen={isFormOpen} onClose={() => setIsFormOpen(false)} title="Tambah Data Baru">
        <form className="space-y-4 mt-4" onSubmit={handleSubmit}>
          ...
          <div><label className="block text-sm font-medium mb-1">Nama Halaqah</label><input type="text" required value={formData.namaHalaqah} onChange={e => setFormData({...formData, namaHalaqah: e.target.value})} className="w-full border rounded-lg p-2" /></div>
          <div><label className="block text-sm font-medium mb-1">Mentor</label><input type="text" required value={formData.mentor} onChange={e => setFormData({...formData, mentor: e.target.value})} className="w-full border rounded-lg p-2" /></div>
          ...
          <div><label className="block text-sm font-medium mb-1">Materi Pembahasan</label><input type="text" required value={formData.materi} onChange={e => setFormData({...formData, materi: e.target.value})} className="w-full border rounded-lg p-2" /></div>
          <div><label className="block text-sm font-medium mb-1">Daftar Hadir Kader (Format bebas / CSV)</label><textarea rows={3} placeholder="Contoh: Budi (Hadir), Andi (Izin)" value={formData.daftarHadir} onChange={e => setFormData({...formData, daftarHadir: e.target.value})} className="w-full border rounded-lg p-2" /></div>
          <div><label className="block text-sm font-medium mb-1">Evaluasi / Catatan</label><textarea rows={2} value={formData.evaluasi} onChange={e => setFormData({...formData, evaluasi: e.target.value})} className="w-full border rounded-lg p-2" /></div>
