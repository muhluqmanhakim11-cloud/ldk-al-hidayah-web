  ...
  return (
    <div className="space-y-8">
      ...
      <div className="bg-gradient-to-b from-blue-50/50 to-white p-6 md:p-8 rounded-2xl border border-blue-100 shadow-sm relative overflow-hidden">
        ...
        <div className="absolute bottom-0 left-0 w-32 h-32 bg-blue-50 rounded-full blur-3xl opacity-50 -ml-10 -mb-10 pointer-events-none"></div>
              ...
              <div key={level} className="flex flex-col items-center w-full">
                <div className="flex flex-wrap justify-center gap-4 md:gap-6 w-full">
                  {bphGrouped[level].map(member => (
                    <div key={member.id} className="bg-white border border-blue-100 p-5 rounded-xl w-full sm:w-64 text-center shadow-[0_2px_10px_-3px_rgba(6,81,237,0.1)] hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)] hover:-translate-y-1 transition-all duration-300">
                      ...
                      <p className="font-bold text-gray-900 line-clamp-1">{member.name}</p>
                      <p className="text-sm font-semibold text-blue-600 mt-1 line-clamp-1">{member.position?.name}</p>
                    </div>
                  ))}
                </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {divisions.map((div: any) => {
          ...
          return (
            <div key={div.id} className="bg-white hover:bg-emerald-50/20 p-6 rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-all duration-300 h-full flex flex-col relative overflow-hidden">
              ...
              <h3 className="text-lg font-bold text-center text-emerald-800 border-b border-gray-100 pb-4 mb-5 flex items-center justify-center">
                {div.name}
              </h3>
              ...
              <div className="flex flex-col items-center space-y-4 flex-grow">
                {divLevels.length === 0 && (
                  <div className="flex flex-col items-center justify-center h-full opacity-50 py-4">
                    <p className="text-sm text-gray-500 italic">Belum ada pengurus</p>
                  </div>
                )}
                ...
                {divLevels.map(level => (
                  <div key={level} className="w-full space-y-3">
                    {divGrouped[level].map(member => (
                      <div key={member.id} className="bg-white border border-emerald-100/80 p-3.5 rounded-xl text-center shadow-[0_1px_3px_rgba(0,0,0,0.04)] hover:shadow-md hover:border-emerald-300 transition-all group">
                        <p className="font-bold text-gray-800 text-sm group-hover:text-emerald-900 transition-colors">{member.name}</p>
